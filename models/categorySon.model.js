...
module.exports = {
    allByCategorySon: (id,limit, offset) => {
        return db.load(`select newspapers.Id, newspapers.Title, newspapers.Thumbnail, newspapers.Created_date, newspapers.Summary, newspapers.Content, newspapers.Count_Like, categories.Name, categorysons.NameSon, users.Pseudonym as Author
                        from newspapers, categories, categorysons, users
                        where newspapers.Id_Author = users.Id and newspapers.CategorySon_id = categorysons.Id and categories.Id = categorysons.Category_id and categorysons.Id = ${id} order by newspapers.Count_Like DESC limit ${limit} offset ${offset}`);
    },
