...
module.exports = {
    allByTag: (name, limit, offset) => {
        return db.load(`select newspapers.Id, newspapers.Title, newspapers.Thumbnail, newspapers.Created_date, newspapers.Summary, newspapers.Content, newspapers.Count_Like, categories.Name, t1.Name as tagName1, t2.Name as tagName2, users.Pseudonym as Author
                        ...
                        where newspapers.Id_Author = users.Id and newspapers.CategorySon_id = categorysons.Id and categories.Id = categorysons.Category_id and newspapers.tag1 = t1.Id and newspapers.tag2 = t2.Id and (t1.Name = '${name}' or t2.Name = '${name}') order by newspapers.Count_Like DESC limit ${limit} offset ${offset}`);
    },
